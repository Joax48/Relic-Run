-- Goblin1: persigue al jugador, melee.
-- Frames: idle=4, walk=6, attack=5, death=6 | 4 filas de dirección (S/W/E/N)
...
local function set_mode(mode, row)
    if cur_mode ~= mode then
        if     mode == "idle"   then play_animation(this, "goblin1-idle",   4, 6)
        elseif mode == "walk"   then play_animation(this, "goblin1-walk",   6, 10)
        elseif mode == "attack" then play_animation(this, "goblin1-attack", 5, 12)
        end
        cur_mode = mode
    end
...
function on_collision(other)
    ...
    if tag == "projectile" or tag == "player_melee" then
        ...
        if HP <= 0 then
            ...
            play_animation(this, "goblin1-death", 6, 6)
